...
## ------------------------------------------------------------------------- ##
##
## Script name:   Precipitation analysis
##
...
#### --------------------------- Exploring the different rainfall datasets #### 
...
#### ------------------- Classify years from GLDAS 2.1 (based on stations) ####

####' ----- Get dry years from GLDAS #### 
(dry_years <- precip_annual %>%
  ...
  mutate(
    Min_Dif = abs(
      min(
        precip_annual %>% 
          filter(!str_detect(Source, 'GLDAS'), Condition == 'Dry') %>% 
          pull(Precip), # Get station precip (minimum value in dry station)
        na.rm = TRUE
        ) 
      - GLDAS2_1 # Subtract by GLDAS precip
      ), 
    Mean_Dif = abs(
      mean(
        precip_annual %>% 
          filter(!str_detect(Source, 'GLDAS'), Condition == 'Dry') %>% 
          pull(Precip), # Get station precip (mean value in dry station)
        na.rm = TRUE
        ) 
      - GLDAS2_1 # Subtract by GLDAS precip
      ),
    Max_Dif = abs(
      max(
        precip_annual %>% 
          filter(!str_detect(Source, 'GLDAS'), Condition == 'Dry') %>% 
          pull(Precip), # Get station precip (max value in dry station)
        na.rm = TRUE
        ) 
      - GLDAS2_1) # Subtract by GLDAS precip
    ))
...
(normal_years <- precip_annual %>%
  ...
  mutate(
    Mean_Dif = abs(
      mean(
        precip_annual %>% 
          filter(!str_detect(Source, 'GLDAS'), Condition == 'Normal') %>% 
          pull(Precip), # Get station precip (mean value of normal years)
        na.rm = TRUE
        ) 
      - GLDAS2_1) # Subtract by GLDAS precip
    ))
...
(wet_years <- precip_annual %>%
  ...
  mutate(
    Min_Dif = abs(
      min(
        precip_annual %>% 
          filter(!str_detect(Source, 'GLDAS'), Condition == 'Wet') %>% 
          pull(Precip), # Get station precip (minimum value in wet station)
        na.rm = TRUE
        ) 
      - GLDAS2_1 # Subtract by GLDAS precip
      ),
    Mean_Dif = abs(
      mean(
        precip_annual %>% 
          filter(!str_detect(Source, 'GLDAS'), Condition == 'Wet') %>% 
          pull(Precip), # Get station precip (mean value in wet station)
        na.rm = TRUE
        ) 
      - GLDAS2_1 # Subtract by GLDAS precip
      ),
    Max_Dif = abs(
      max(
        precip_annual %>% 
          filter(!str_detect(Source, 'GLDAS'), Condition == 'Wet') %>% 
          pull(Precip), # Get station precip (max value in wet station)
        na.rm = TRUE
        ) 
      - GLDAS2_1) # Subtract by GLDAS precip
    ))

#### ------------------- Analyze station and GLDAS years ####
...
####' ----- Plot intra-annual variation of precip (both sources)  #### 
precip_monthly %>%
...
(meteo_classes <- precip_annual %>%
  filter(!str_detect(Source, 'GLDAS')) %>%
  drop_na() %>%
  group_by(Condition) %>%
  summarise(Mean = mean(Precip), Min = min(Precip), Max = max(Precip)) %>%
  gather(key = Metric, value = Precip, -Condition) %>% 
  mutate(Precip = if_else( # Remove min/max from "Normal" years quantile range
...
#### ------------------------------------- Create scenarios representation ####
